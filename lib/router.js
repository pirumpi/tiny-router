var http = require('http'),
    path = require('path'),
    router = function(){ this.routes = []; this.mdware = []; this.methods = ['GET', 'POST', 'PUT', 'DELETE']; this.init(); this.static = null; this.defaultPage = 'index.html'; this.fs = require('fs');
...
router.prototype.sendImageFn = function(){
    ...
    this.writeHead(200, {'Content-Type': contentType, 'Content-Length': image.length, 'Access-Control-Allow-Origin':'*'});
    this.end(image);
};
/*Users can overwrite this method to use the embed their embed system API*/
router.prototype.fileExist = function(file, cb){ this.fs.exists(file, function(exists){ cb(exists); }); };

/*Users can overwrite this method to use the embed their embed system API*/
router.prototype.readFile = function(file, cb){ this.fs.readFile(file, function(err, data){ cb(err, data); }); }
...
var findPath = function(req, res, router, cb){
    ...
    augmentedFn = function(req, res, router){
        res.send = function(){ router.sendFn.apply(res, arguments); }
        res.sendImage = function(){ router.sendImageFn.apply(res, arguments); }
    },
...
router.prototype.Router = function(){
    ...
    return function(req, res){
        ...
        if(mdware.length){
            middleware(mdware, req, res, 0, function(){ routeFn(req, res, router); });
        }else{
